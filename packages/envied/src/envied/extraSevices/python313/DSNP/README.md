...
- To enable the TV pair code login method, please comment out or delete the DSNP section under credentials in `envied.yaml`.
  TV 페어 코드 로그인 방식을 활성화하려면 `envied.yaml`의 credentials에서 DSNP부분을 주석 처리하거나 삭제하세요.
